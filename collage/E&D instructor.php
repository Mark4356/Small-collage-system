...
if(isset($_GET["ID"])){
    $id=$_GET["ID"];
$selectinstructors="SELECT * FROM `instructor` WHERE id = $id";
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $NS=$_POST["selmnue"];
    // $name=$_POST["NN"];
    $updateInstructors="UPDATE `instructor` SET `ename` = '$NN',`email` = '$NE',`hire_date`= '$NH', department_id ='$NS' WHERE `id` = $id";
    $instructorup = mysqli_query($conn,$updateInstructors);
    header("Location:http://localhost/backend%20project/collage/E&D%20instructor.php?ID=$id");
}
...
<body>
    ...
    <h1 class="text-center display-6 my-4 fw-bold">Show Instructor
...
<h2 class="text-start display-6 my-4 fw-bold back-dark">
    <?=$instructorData["ename"] ?><br>
    <?=$instructorData["email"] ?><br>
    <?=$instructorData["hire_date"] ?><br>
    <?=$qd["title"] ?><br>
    </h2>
</div>
<form action="" method="POST">
    ...
    <input type="text" class="form-control" name="NN" value ="<?=$instructorData['ename'];?>">
    ...
    <input type="text" class="form-control" name="NH" value="<?=$instructorData['hire_date'];?>">
